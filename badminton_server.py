        ...
        while not thread_stop_event.isSet():
            with app.app_context():
                ...
                for code, string in location_dict.items():
                    res = json.loads(
                        requests.get(raw_booking_url.format(id=code,startdate=start_date,enddate=end_date), headers=user_header).text
                    )
                            ...
                            if i['status'] == "Available":
                                ...
                                if work_time_filter:
                                    if start_time <= time_obj <= end_time:
                                        datetime_obj = datetime.strptime(i["date"] + " " + i["start_time"], "%Y-%m-%d %H:%M")
